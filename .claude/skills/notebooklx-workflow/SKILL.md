...
1. run `/compact` before starting the next slice
2. then begin the next unfinished feature slice with fresh, minimal context

If `/compact` is unavailable in the current client, create a minimal handoff summary that includes only:
...
- Do not mirror client permission metadata into workflow docs.

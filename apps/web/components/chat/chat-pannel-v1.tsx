...
export function ChatPanel({ notebookId, notebookName, variant = "grounded" }: ChatPanelProps) {
  ...
  const latestAssistantMessageWithQueryRewrite =
    (streamingMessage?.queryRewrite?.rewritten ? streamingMessage : null) ||
    [...messages].reverse().find((message) => message.role === "assistant" && Boolean(message.queryRewrite?.rewritten));
  ...
  return (
    <Card className={cardClasses}>
      ...
      <CardContent className={cardContentClasses}>
        ...
        <div ref={scrollContainerRef} className={conversationPaneClasses}>
          {messages.length === 0 ? (
